export default class VideoProcessor {
       ...
       start: async(controller) => {
        ...
        return this.#mp4Demuxer.run(stream, {
          async onConfig(config) {
            ...
            // if(!supported) {
            //   console.error('mp4Muxer VideoDecoder config not supported')
            //   controller.close()
            //   return
            // }
  ...
  encoder144p(encoderConfig) {
    let _encoder;
    const readable = new ReadableStream({
      start: async(controller) => {
        const { supported } = await VideoEncoder.isConfigSupported(encoderConfig)
        if(!supported) {
          const message = 'encoder144p VideoEncoder config not supported'
    ...
    return new TransformStream({
      start(controller) {
        _decoder = new VideoDecoder({
          ...
          error(e) {
            console.error('error at renderFrames')
            controller.error(e)
          }
        })
      },
  ...
  upload(filename, resolution, type) {
    ...
    return new WritableStream({
      ...
      async write({ data }) {
        ...
        // Se for menor q 10m não faz upload
  ...
  async start({ file, encoderConfig, renderFrame, sendMessage }) {
    ...
    await this.mp4Decoder(stream)
      .pipeThrough(this.encoder144p(encoderConfig))
